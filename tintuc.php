<div id="tintuc">
	...
	/*------------Phan trang------------- */
		...
		// Chọn số kết quả trả về trong mỗi trang mặc định là 6
		$max_results = 6;  
		...
		$sql = mysqli_query($connect,"SELECT * FROM tintuc ORDER by matt DESC  LIMIT $from, $max_results"); 
...
<div id="phantrang_sp">
	...
	<?php
			...
			$total_results = mysqli_num_rows(mysqli_query($connect,"SELECT * FROM tintuc"));  
			...
			if($page > 1){  
			$prev = ($page - 1);  
			echo "<a href=\"".$_SERVER['PHP_SELF']."?content=tintuc&page=1\"><button class='trang'><<</button></a>&nbsp;";  
			echo "<a href=\"".$_SERVER['PHP_SELF']."?content=tintuc&page=$prev\"><button class='trang'><</button></a>&nbsp;";    
			}  
			...
			if($page < $total_pages){  
			$next = ($page + 1);  
			echo "<a href=\"".$_SERVER['PHP_SELF']."?content=tintuc&page=$next\"><button class='trang'>></button></a>";  
			echo "<a href=\"".$_SERVER['PHP_SELF']."?content=tintuc&page=$total_pages\"><button class='trang'>>></button></a>";  
			}  
			echo "</center>";  		
